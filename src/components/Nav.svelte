<script>
	...
	/**
	 * @type {{ minRulesetLength: number; maxRulesetLength: number; minInputStringLength: number; maxInputStringLength: number; allowPartialINR: any; allowPartialTRM: any; allowPartialOutput: any; }}
	 */
	export let settings;
...
<nav class="flex fixed justify-between items-center w-full h-14 bg-gray-300 px-4 z-10 top-0">
	<div class="text-xl font-bold">Search & Change</div>
	<button class="flex w-10 h-10 justify-center items-center relative bg-gray-300" on:click={toggleMenu}>
		<i class="bi bi-gear-fill text-xl" />
		<div class="fixed {menuIsOpen ? 'flex flex-col' : 'hidden'} p-4 top-14 right-0 bg-gray-200 w-screen sm:w-96 rounded border shadow hover:cursor-default items-start z-10 gap-2" on:click={handleDivClick} role="button" tabindex="0" on:keydown={handleDivClick}>
